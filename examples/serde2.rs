...
struct User {
    ...
    data: Vec<u8>,
    // serialize_with works when sensitive is a string
    // #[serde(
...
fn encrypt(data: &[u8]) -> Result<String> {
    ...
    let nounce_ciphertext = nonce.iter().copied().chain(ciphertext).collect::<Vec<_>>();
    let encoded = URL_SAFE_NO_PAD.encode(nounce_ciphertext);
    Ok(encoded)
}
